...
def generate_launch_description():
    ...
    mode         = LaunchConfiguration('mode', default = 'rgb')
    ...
    rgbScaleNumerator       = LaunchConfiguration('rgbScaleNumerator',  default = 16)
    rgbScaleDinominator     = LaunchConfiguration('rgbScaleDinominator',    default = 16)
    ...
    useRGBSegmentation = LaunchConfiguration('useRGBSegmentation', default='false')
    segmentationConfigPath = LaunchConfiguration('segmentationConfigPath', 
        default=os.path.join(get_package_share_directory('scooter_segmentation'), 
                            'resources/config_65.json'))
    ...
    declare_useRGBSegmentation_cmd = DeclareLaunchArgument(
        'useRGBSegmentation',
        default_value=useRGBSegmentation,
        description='When true, publishes RGB segmentation masks using the color mapping from config file')

    declare_segmentationConfigPath_cmd = DeclareLaunchArgument(
        'segmentationConfigPath',
        default_value=segmentationConfigPath,
        description='Path to the JSON file containing segmentation color mappings')
    ...
    fastscnn_node = launch_ros.actions.Node(
            ...
            parameters=[{'mxId':                    mxId},
                        ...
                        {'useRGBSegmentation': useRGBSegmentation},
                        {'segmentationConfigPath': segmentationConfigPath}
                        ])
    ...
    ld.add_action(declare_useRGBSegmentation_cmd)
    ld.add_action(declare_segmentationConfigPath_cmd)
